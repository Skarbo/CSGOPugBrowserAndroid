...
public class RestClient {
	...
	private static final int connectionTimeOutSec = 10;
	private static final int socketTimeoutSec = 10;
	...
	private Response executeRequest(HttpUriRequest request, String url) throws IOException {
		...
		Log.d(TAG, "Client execute: Method: " + request.getMethod() + ", URI: " + request.getURI().toString());
		// for (NameValuePair pair : data) {
		// Log.d(TAG, "Data: Name: " + pair.getName() + " Value: " +
		// pair.getValue());
		// }
		// for (NameValuePair pair : headers) {
		// Log.d(TAG, "Header: Name: " + pair.getName() + " Value: " +
		// pair.getValue());
		// }
			...
			for (Header header : httpResponse.getAllHeaders()) {
				if (header.getName().equalsIgnoreCase("Set-Cookie")) {
					for (HeaderElement headerElement : header.getElements()) {
						BasicClientCookie cookie = new BasicClientCookie(headerElement.getName(),
								headerElement.getValue());

						if (headerElement.getParameterByName("path") != null)
							cookie.setPath(headerElement.getParameterByName("path").getValue());
	...
	public static class Response {
		...
		public String toString() {
			return String.format("Code: %d, Message: %s, Response: %d", this.getResponseCode(), this.getMessage(), this
					.getResponse().length());
		}
